...
## [1.2.3] - 2025-12-24
### 修复 & 优化
- **全量逻辑回归 (Full Logic Regression)**：响应用户反馈，撤销了所有过度干预的碎片化函数，将脚本结构恢复为原项目的单块 `installTrojan` 流程，极大提升了脚本在不同环境下的执行一致性。
- **修正 acme.sh 安装源**：重新指向官方 `get.acme.sh` 通道，仅保留必要的 `--install-online` 环境补丁，解决 Ubuntu 24.04 路径定位故障。
- **环境自愈补丁**：自动清理旧版本留下的错误 Service 配置文件，引导系统回归原生 Docker 启动链。
- **精简依赖**：移除所有非必要的 `FATAL` 退出检查，允许主程序在系统原生 Docker 环境下运行。
