...
## [1.2.2] - 2025-12-24
### 优化
- **逻辑回归原生 (Keep It Simple)**：大幅简化了 Docker 的处理逻辑，移除了之前过于激进的“核能级清理”和“静态二进制覆盖”。回归使用系统原生的包管理器（apt/yum）来安装和准备环境。
- **环境自愈补丁**：仅保留了对上个测试版本中错误 Service 配置文件的专项清理，确保系统能够恢复到默认的 Docker 启动链路。
- **保持核心修复**：保留了 acme.sh 安装补丁及数据库 API 版本适配。
